...
public class Timer {
	...
	static public synchronized Timer instance () {
		if (instance == null)
			instance = new Timer();
		return instance;
	}
	...
	public synchronized Task scheduleTask (Task task, float delaySeconds, float intervalSeconds, int repeatCount) {
		if (task.timer != null)
			throw new IllegalArgumentException("The same task may not be scheduled twice.");
		task.timer = this;
		long timeMillis = System.nanoTime() / 1000000;
		task.executeTimeMillis = timeMillis + (long)(delaySeconds * 1000);
